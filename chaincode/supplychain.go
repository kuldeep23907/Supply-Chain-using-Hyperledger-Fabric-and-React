...
func (t *food_supplychain) createProduct(APIstub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	var comAsset = Product{Product_Id: "Product" + strconv.Itoa(productCounter), Order_ID: nil, Name: args[0], Consumer_ID: nil, Manufacturer_ID: args[1], Retailer_ID: nil, Distributer_ID: nil, Wholesaler_ID: nil, Status: "Available", Date: dates.ManufactureDate, Product_Price: i1}
...
func deliveredProduct(APIstub shim.ChaincodeStubInterface,args[] string) pb.Response{
	// parameter length check
	if len(args) != 1 {
		return shim.Error("Incorrect number of arguments, Required 4")
	}
	...
	dates.DeliveredDate = txTimeAsPtr
...
func (t *food_supplychain) queryAll(APIstub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	endKey := assetType + strconv.Itoa(assetCounter)

	resultsIterator, err := APIstub.GetStateByRange(, endKey)
