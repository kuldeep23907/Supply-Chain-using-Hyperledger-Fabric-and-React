...
chaincodeInvoke() {
    ...
    "manager")
        ...
        ;;
    "register")
        ARGS='{"Args":["register","{\"surveyKey\":\"전남대학교_0\",\"surveyInfo\":{\"class\":\"org.example.surveyinfo\",\"key\":\"org.example.surveyinfo:전남대학교:0\",\"currentState\":null,\"department\":\"전남대학교\",\"createdAt\":0,\"managerID\":\"admin\",\"title\":\"test\",\"startDate\":0,\"finishDate\":0},\"questions\":[{\"class\":\"org.example.surveyquestion\",\"key\":\"org.example.surveyquestion:전남대학교_0:0\",\"surveyKey\":\"전남대학교_0\",\"questionNum\":0,\"title\":\"test\",\"type\":\"text\",\"contents\":[]}]}"]}'
        ;;
    "remove")
    ...
    if [ -z $CORE_PEER_TLS_ENABLED -o $CORE_PEER_TLS_ENABLED = "false" ]; then
        set -x
        peer chaincode invoke -o orderer.example.com:7050 -C $CHANNEL_NAME -n $CC_NAME $PEER_CONN_PARMS -c $ARGS >&log.txt
        res=$?
        set +x
    else
        set -x
        peer chaincode invoke -o orderer.example.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n $CC_NAME $PEER_CONN_PARMS -c $ARGS >&log.txt
        res=$?
        set +x
    fi
    sleep $DELAY
